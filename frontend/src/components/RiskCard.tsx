...
import { Thermometer, ShieldAlert, Cpu, MapPin, Truck } from 'lucide-react';
  ...
  return (
    <div className="glass-panel rounded-2xl p-5 border border-slate-800 h-full flex flex-col justify-between">
      <div>
        ...
        <div className="my-4 p-4 rounded-xl bg-slate-900/90 border border-slate-800">
          ...
          <div className="grid grid-cols-2 gap-3 text-xs mt-3">
            <div>
              <p className="text-slate-400 flex items-center gap-1"><MapPin size={12} /> {t('location')}</p>
              <p className="font-semibold text-slate-200 mt-0.5">{shipment.origin} → {shipment.destination}</p>
            </div>

            <div>
              <p className="text-slate-400 flex items-center gap-1"><Thermometer size={12} /> {t('temp')}</p>
        ...
        {risk ? (
          <div className="p-4 rounded-xl bg-slate-900/60 border border-slate-800/80">
            <p className="text-xs font-semibold text-slate-400 uppercase tracking-wider mb-2 flex items-center gap-1.5">
              <ShieldAlert size={14} className="text-amber-400" /> {t('reasoning_engine')}
            </p>
            ...
            <div className="mt-3 pt-3 border-t border-slate-800/60 flex items-center justify-between text-[11px] font-mono text-slate-400">
              <span>{t('category')}: <strong className="text-white uppercase">{risk.risk_category}</strong></span>
              <span>{t('confidence')}: <strong className="text-emerald-400">{(risk.confidence * 100).toFixed(0)}%</strong></span>
            </div>
          </div>
        ) : (
          <div className="p-6 text-center text-slate-500 text-xs font-mono">
            {t('select_shipment_map')}
          </div>
        )}
      ...
      <div className="pt-4 border-t border-slate-800 text-[11px] text-slate-400 flex justify-between font-mono">
        <span>{t('cargo_value')}: ${shipment.value_usd.toLocaleString()}</span>
        <span>{t('status')}: <strong className="text-blue-400 uppercase">{shipment.status}</strong></span>
      </div>
    </div>
  );
};
